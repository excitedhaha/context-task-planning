...
Requirements:
- Treat `$ARGUMENTS` as an already-confirmed task title and pass it to `sh "${COCO_PLUGIN_ROOT}/skill/scripts/init-task.sh"`.
...
```bash
sh "${COCO_PLUGIN_ROOT}/skill/scripts/init-task.sh" "$ARGUMENTS"
```
