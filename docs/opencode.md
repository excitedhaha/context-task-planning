...
```bash
opencode plugin context-task-planning-opencode@0.8.5 --global --force
```
...
- if the title/toast behavior is missing but slash commands are present, rerun `sh ~/.config/opencode/skills/context-task-planning/scripts/install-opencode-plugin.sh` (or `opencode plugin context-task-planning-opencode@0.8.5 --global --force`)
