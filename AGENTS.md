...
- Keep routing, drift, repo, worktree, and context-prune logic in the shared core under `skill/scripts/`; `task_guard.py` is the CLI facade, while focused modules such as `task_drift.py`, `task_preflight.py`, `task_prune.py`, `spec_context.py`, and `task_text.py` own reusable decisions.
