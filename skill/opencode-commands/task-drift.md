...
- Run `sh "$HOME/.config/opencode/skills/context-task-planning/scripts/check-task-drift.sh" --prompt "$ARGUMENTS" --json` from the current workspace.
...
```bash
sh "$HOME/.config/opencode/skills/context-task-planning/scripts/check-task-drift.sh" --prompt "$ARGUMENTS" --json
```
