...
if [ -z "$PLAN_DIR" ] || [ ! -d "$PLAN_DIR" ]; then
    echo "[context-task-planning] No task found to cancel delegate in." >&2
    exit 1
fi
...
if [ -z "$PYTHON_BIN" ]; then
    echo "Python is required to cancel delegates." >&2
    exit 1
fi
...
if existing_status == "cancelled":
    raise SystemExit("Delegate is already cancelled.")
if existing_status == "complete":
    raise SystemExit("Cannot cancel a completed delegate. Leave it complete or use a new delegate id.")

if not delegate_summary:
    delegate_summary = delegate_state.get("summary", "").strip() or "Delegate was cancelled and should not block the main task anymore."

delegate_state["status"] = "cancelled"
