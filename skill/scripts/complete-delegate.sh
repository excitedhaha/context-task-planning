...
if [ -z "$PLAN_DIR" ] || [ ! -d "$PLAN_DIR" ]; then
    echo "[context-task-planning] No task found to complete delegate in." >&2
    exit 1
fi
...
if [ -z "$PYTHON_BIN" ]; then
    echo "Python is required to complete delegates." >&2
    exit 1
fi
...
export STATE_FILE PROGRESS_FILE DELEGATE_STATUS_FILE DELEGATE_RESULT_FILE DELEGATE_ID DELEGATE_SUMMARY ALLOW_MAIN_PLAN_WRITES
"$PYTHON_BIN" <<'PY'
import json
import os
...
if state.get("status") == "archived":
    raise SystemExit("Cannot complete a delegate for an archived task.")

existing_status = delegate_state.get("status")
if existing_status == "cancelled":
    raise SystemExit("Cannot complete a cancelled delegate.")
if existing_status == "complete":
    raise SystemExit("Delegate is already complete.")

if not delegate_summary and delegate_result_path.exists():
    delegate_summary = extract_summary(delegate_result_path.read_text(encoding="utf-8"))

if not delegate_summary:
    delegate_summary = "Delegate completed. Review result.md for details."

delegate_state["status"] = "complete"
...
if allow_main_plan_writes:
    ...
    state["latest_checkpoint"] = f"Delegate {delegate_id} completed at {timestamp}."
...
if allow_main_plan_writes and progress_path.exists():
    with progress_path.open("a", encoding="utf-8") as fh:
        fh.write("\n")
        fh.write(f"### Delegate Complete: {timestamp}\n\n")
        fh.write(f"- Delegate: `{delegate_id}`\n")
        fh.write("- Status: complete\n")
        fh.write(f"- Summary: {delegate_summary}\n")
        fh.write("- Notes:\n")
        fh.write("  - Delegate completed via complete-delegate.sh\n")
PY
...
echo "[context-task-planning] Completed delegate: $DELEGATE_ID"
...
echo "[context-task-planning] Next step: review result.md and run promote-delegate.sh if the findings should be merged back."
