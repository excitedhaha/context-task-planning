...
- `task_prune.py`: oversized `progress.md` detection, prune brief generation, archive/apply/restore logic, and prune status formatting.
...
- Prefer moving pure logic into `task_text.py`, `spec_context.py`, `task_drift.py`, `task_preflight.py`, or `task_prune.py` before adding more code to `task_guard.py`.
